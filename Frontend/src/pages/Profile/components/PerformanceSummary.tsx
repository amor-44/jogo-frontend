...
export const PerformanceSummary = ({ report }: PerformanceSummaryProps) => {
  if (!report) {
    return (
      <div className="bg-white p-8 rounded-3xl shadow-2xs border border-gray-100 text-center">
  ...
  const items = [
    { label: 'دقة التمرير', value: metrics.passingAccuracy ?? metrics.passing ?? 0, color: 'bg-blue-600' },
    { label: 'التحكم بالكرة / المراوغة', value: metrics.ballControl ?? metrics.dribbling ?? 0, color: 'bg-[#2B43A1]' },
    { label: 'التمركز والرؤية', value: metrics.positioningScore ?? metrics.positioning ?? 0, color: 'bg-emerald-600' },
    { label: 'كفاءة الحركة والسرعة', value: metrics.movementEfficiency ?? metrics.speed ?? 0, color: 'bg-indigo-600' },
    { label: 'المهارات الدفاعية', value: metrics.defensiveActions ?? metrics.defending ?? 0, color: 'bg-amber-600' },
    { label: 'التأثير الهجومي', value: metrics.attackingImpact ?? metrics.shooting ?? 0, color: 'bg-purple-600' },
  ].filter(i => i.value > 0);
    ...
    <>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white p-5 rounded-3xl shadow-2xs border border-gray-100 text-center flex flex-col items-center justify-center">
          ...
          <span className="text-3xl font-extrabold text-[#2B43A1] mb-2">{overall}</span>
        ...
        <div className="bg-[#EBFDF5] p-5 rounded-3xl border border-emerald-100 text-center flex flex-col items-center justify-center">
          ...
          <span className="text-emerald-600 text-[10px]">تقييم الأداء العام</span>
        </div>

        <div className="bg-white p-5 rounded-3xl shadow-2xs border border-gray-100 text-center flex flex-col items-center justify-center">
          <span className="text-gray-400 text-xs font-medium mb-1">تاريخ آخر تحليل</span>
      ...
      <div className="bg-white p-6 rounded-3xl shadow-2xs border border-gray-100">
        <div className="flex justify-between items-center mb-6">
          <h3 className="font-bold text-gray-900 text-base flex items-center gap-2">
            <BarChart3 className="w-5 h-5 text-[#2B43A1]" /> ملخص الأداء الفعلي
          </h3>
        </div>
        ...
        <div className="space-y-5">
          ...
          {items.length === 0 && (
            <p className="text-xs text-gray-400 text-center py-4">لا تتوفر تفاصيل إحصائية رقمية في هذا التقرير</p>
          )}
        </div>
      </div>
    </>
