...
public class RefreshCommandHandler(
    ...
    ILogger<RefreshCommandHandler> logger) : IRequestHandler<RefreshCommand, Result<AuthResponse>>
{
    public async Task<Result<AuthResponse>> Handle(RefreshCommand request, CancellationToken cancellationToken)
    {
        ...
        // 6. إرجاع النتيجة
        return new AuthResponse(newAccessToken, newRefreshToken, user.Role.ToString(), user.Id);
    }
}
