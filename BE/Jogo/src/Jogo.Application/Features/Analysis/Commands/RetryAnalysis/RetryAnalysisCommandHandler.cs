...
public class RetryAnalysisCommandHandler : IRequestHandler<RetryAnalysisCommand, Result<Success>>
{
    private readonly IAppDbContext _context;
    private readonly IUser _currentUser;
    private readonly IBackgroundJobService _backgroundJobService;

    public RetryAnalysisCommandHandler(
    ...
    public async Task<Result<Success>> Handle(RetryAnalysisCommand request, CancellationToken cancellationToken)
    {
        ...
        if (video.PlayerProfileId != profile.Id)
            return Error.Forbidden("Analysis.Forbidden", "You can only retry analysis for your own videos.");

        var retryResult = video.Retry();
        if (retryResult.IsError)
            return retryResult.Errors;
